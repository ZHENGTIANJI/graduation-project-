...
public partial class delete_journaldelete : System.Web.UI.Page
{
    ...
    void ExecuteQuery()
    {
        ...
        else
        {
            ...
            try
            {
                DataSet dst = new DataSet();
                SqlConnection cnn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["dtcnn"].ConnectionString);
                SqlDataAdapter adpt = new SqlDataAdapter("SELECT [id],[number_of_page], [office_name], [period_type],[period_number], [address],[format], [location], [stock_dt], [number], [name],[publish_dt], [jtname], [mname], [QR_code], [class_number],[organname], [status], [note], [lname],[gradename] FROM [journal] left join journal_grade on journal.gradeid=journal_grade.gid left join organization on journal.organizationid=organization.organid left join journal_type on journal.journal_type_id=journal_type.jtid left join major on journal.major_id=major.mid left join language on journal.language=language.lid where stock_dt<='" + date + "'", cnn);
                adpt.Fill(dst);
                if (dst.Tables[0].Rows.Count == 0)
                {
                    DataRow row = dst.Tables[0].NewRow();
    ...
    protected void GridView1_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        if (GridView1.Rows[0].Cells[0].Text.ToString() != GridView1.Rows[0].Cells[2].Text.ToString() && GridView1.Rows[0].Cells[2].Text.ToString() != GridView1.Rows[0].Cells[1].Text.ToString())
        {
            ...
            int num = e.RowIndex;
            int journalid = Convert.ToInt32(GridView1.Rows[num].Cells[20].Text.ToString());
            try
            {
                cmd.CommandText = "update journal set is_delete='" + 1 + "' where id=" + journalid + "";
                cnn.Open();
                cmd.ExecuteNonQuery();
                cnn.Close();
                ExecuteQuery();
    ...
    protected void GridView2_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        if (GridView2.Rows[0].Cells[0].Text.ToString() != GridView2.Rows[0].Cells[1].Text.ToString() && GridView2.Rows[0].Cells[1].Text.ToString() != GridView2.Rows[0].Cells[2].Text.ToString())
        {
            ...
            int num = e.RowIndex;
            int journalid = Convert.ToInt32(GridView2.Rows[num].Cells[20].Text.ToString());
            try
            {
                cmd.CommandText = "update journal set is_delete='" + 0 + "' where id=" + journalid + "";
                cnn.Open();
                cmd.ExecuteNonQuery();
                cnn.Close();
                ExecuteQuery();
    ...
    void ExecuteQuery2()
    {
        ...
        SqlConnection cnn = new SqlConnection(System.Configuration.ConfigurationManager.ConnectionStrings["dtcnn"].ConnectionString);
        SqlDataAdapter adpt1 = new SqlDataAdapter("SELECT [id],[number_of_page], [office_name], [period_type],[period_number], [address],[format], [location], [stock_dt], [number], [name],[publish_dt], [jtname], [mname], [QR_code], [class_number],[organname], [status], [note], [lname],[gradename] FROM [journal] left join journal_grade on journal.gradeid=journal_grade.gid left join organization on journal.organizationid=organization.organid left join journal_type on journal.journal_type_id=journal_type.jtid left join major on journal.major_id=major.mid left join language on journal.language=language.lid where is_delete=1 and name like '%"+TextBox2.Text.ToString()+"%'", cnn);
        adpt1.Fill(dst1);
    ...
    {
        if (this.GridView2.PageIndex < this.GridView1.PageCount)
        {
            this.GridView2.PageIndex++;
            ExecuteQuery2();
        }
