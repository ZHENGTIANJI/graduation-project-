...
public partial class stockmanagement_journalstock : System.Web.UI.Page
{
    ...
    void ExecuteQuery()
    {
        ...
        SqlDataAdapter adpt = new SqlDataAdapter("SELECT [id],[number_of_page], [office_name], [period_type],[period_number], [address],[format], [location], [stock_dt], [number], [name],[publish_dt], [jtname], [mname], [QR_code], [class_number],[organname],case when [status]=0 then '库存' when [status]=1 then '预约' else '借出' end status, [note], [lname],[gradename] FROM [journal] left join journal_grade on journal.gradeid=journal_grade.gid left join organization on journal.organizationid=organization.organid left join journal_type on journal.journal_type_id=journal_type.jtid left join major on journal.major_id=major.mid left join language on journal.language=language.lid where is_delete=0 and stock_dt>='" + DateTime.Now.ToString("yyyy-MM-dd") + "'", cnn);
        adpt.Fill(dst);
