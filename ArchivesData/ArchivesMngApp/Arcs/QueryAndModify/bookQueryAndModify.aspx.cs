...
public partial class QueryAndModify_bookQueryAndModify :dropdownlist
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            SqlDataAdapter adpt = new SqlDataAdapter("SELECT [id],[number_of_page], [book_number], [CD], [format], [location], [stock_dt], [teacher], [number], [name], [btname], [mname], [QR_code], [publisher], [publish_dt], [author], [price], case when [status]=0 then '库存' when [status]=1 then '预约' else '借出' end status , [note], [lname], [bitname] FROM [book] left join book_type on book.book_type_id=book_type.btid left join major on book.major_type_id=major.mid left join language on book.language=language.lid left join binding_type on book.binding_type_id=binding_type.bitid where is_delete=0 and name like '%" + strbookname + "%' and mname like '%" + strmajor + "%' and btname like '%" + strbooktype + "%'", cnn);
            adpt.Fill(dst);
            ...
            SqlDataAdapter adpt1 = new SqlDataAdapter("select * from publisher", cnn);
            adpt1.Fill(dst1);
            ListItem mt = new ListItem();
            mt.Value = "  ";
            mt.Text = "  ";
            ddlModel.Items.Add(mt);
            for (int i = 0; i < dst1.Tables[0].Rows.Count; i++)
            {
                ListItem mt1 = new ListItem();
                mt1.Value = dst1.Tables[0].Rows[i][0].ToString();
                mt1.Text = dst1.Tables[0].Rows[i][1].ToString();
                ddlModel.Items.Add(mt1);
            }
        }
    ...
    {
        txtpub.Text = ddlModel.SelectedItem.Text.ToString();
    }
